...
G_J_12k.ped<- read.csv("data/plink/merged_update_1000G_J_Afa.ped", sep = ' ', header=FALSE, stringsAsFactors=FALSE, 
                       colClasses = c("character"))

mapa_ALL_1000GP <- read.csv("data/plink/merged_update_1000G_J_Afa.map", sep = '', header=FALSE, stringsAsFactors=FALSE, 
                            colClasses = c("character"))
...
analyze3 <- function(file1) {
  ...
  write.csv(  df,'PGS_1000G_J_Afa_9k.csv',row.names = FALSE)
  book1<<-data.frame(df)
...
#exploring the ancient genomes PGS values for 9128 snps
